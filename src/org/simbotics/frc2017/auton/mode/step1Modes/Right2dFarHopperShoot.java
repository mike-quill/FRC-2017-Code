...
public class Right2dFarHopperShoot implements AutonMode{
	...
	public void addToMode(AutonBuilder ab) {
		...
		ab.addCommand(new ShooterSetRPM(RobotConstants.ADJUSTABLE_RPM+500, 0.0));
		ab.addCommand(new SetBallRamp(true));
		
		ab.addCommand(new Drive2dProfile("RightFarHopperCurve",false, 8, 0.01, 2850));
		ab.addCommand(new DriveWait());
		
		ab.addCommand(new TurretAim(10, 3000));
		ab.addCommand(new DriveSetHighGear(false));
		ab.addCommand(new DriveStraightProfile(8 / 12.0, 0, 0.01, 600));
		
		
		ab.addCommand(new AutonWait(600));
		
		ab.addCommand(new AutonOverride(RobotComponent.SHOOTER));
		ab.addCommand(new ShooterShootAndFeedTimer(RobotConstants.ADJUSTABLE_RPM+500,700,700,12000,false,-1));	
